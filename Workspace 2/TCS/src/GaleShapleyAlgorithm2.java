public class GaleShapleyAlgorithm2 {
		...
		for(k=0;k<5;k++)
			//Finding whether Male is free or not 
			 ...
			 {	//checking mproposed
				 for(r=0;r<5;r++)
				 {
					if(mproposed[1][r]==(fPref[k][j]))
					 {
						 count2=Integer.valueOf(mproposed[2][r]);
						 for(m=0;m<5;m++)
							 if(fPref[k][j].equals(mPref[m][0]))
							 for(i=1;i<6;i++)
							 if(fPref[k][0].equals(mPref[m][i]))
							 count3=i;
					 }
				 }
				 if(count2<=count3)
				 {
				 for(l=0;l<5;l++)
				 if(fPref[k][j].equals(fprop[0][l]))
				 if(fprop[1][l].equals(String.valueOf(1)))
				 {
				 for(m=0;m<5;m++)
					 if(fPref[k][j].equals(mPref[m][0]))
					 for(n=1;n<6;n++)
					 if(fPref[k][0].equals(mPref[m][n]))
						 {
							 mproposed[1][k]=fPref[k][j];
				 ...
				 {	 
					 ...
					 {
					 for(n=0;n<5;n++){
	   				 if(mproposed[1][n].equals(fPref[k][j]))
	   				 {
	   				 value=mproposed[0][n];
	...
	public static void main(String[] args) {
		GaleShapleyAlgorithm2 a=new GaleShapleyAlgorithm2();
		a.mprefrences();
		a.proposed();
		a.computation();
		a.display();
